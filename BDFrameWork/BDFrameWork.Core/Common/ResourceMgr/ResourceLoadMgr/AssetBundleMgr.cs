...
namespace BDFramework.ResourceMgr
{
    ...
    public class AssetBundleMgr : IResMgr
    {
...
#endif
            path = path.Replace("\\", "/");
            IEnumeratorTool.IStartCoroutine(IELoadAssetBundles(path, callback, true));
        }
        ...
        public void LoadAssetBundleAsync(string path, Action<bool> sucessCallback)
        {
            ...
            //开始加载队列
            IEnumeratorTool.IStartCoroutine(IELoadAssetBundles(resQue, sucessCallback));
        }
        ...
        IEnumerator IELoadAssetBundles(Queue<string> resQue, Action<bool> callback)
        {
            ...
            if (www.error == null)
            {
                if (www.isDone)
                {
                    ...
                    //刷出
                    IEnumeratorTool.IStartCoroutine(IELoadAssetBundles(resQue, callback));
                    yield break;
                }
            }
            else
