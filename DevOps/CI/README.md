...
每次实现或修改 `DevOps/CI/` 内功能时，先读本文档，再读具体模块 README；如果改动涉及 TeamCity，再继续读 `.test-DevOps/README.md` 和 `.github/skills/teamcity/SKILL.md`。

## 阅读顺序

1. 先读本文档，确认公共规范和验证要求。
2. 再读当前模块 README，确认模块职责、主流程、测试入口和模块补充约束。
3. 如果改动涉及 TeamCity DSL、构建命令、参数透传、CI 日志或远端执行验证，再读 `../../.test-DevOps/README.md` 和 `../../.github/skills/teamcity/SKILL.md`。
...
- TeamCity 执行与排查入口统一维护在 `../../.test-DevOps/README.md` 和 `../../.github/skills/teamcity/SKILL.md`。
