...
namespace BDFramework.ResourceMgr.V2
{
    ...
    public class AssetBundleMgrV2 : IResMgr
    {
        ...
        /// 特殊的前缀
        /// </summary>
        static public string RUNTIME = "runtime/{0}";
        /// <summary>
        /// 异步任务颗粒度，每帧执行多少个
        /// </summary>
        static public int ASYNC_TASK_UNIT = 5;
        /// <summary>
        /// 全局的任务id
        /// </summary>
        ...
        {
            //
            var mainItem = config.Manifest.GetManifest(name);
            //单ab 多资源,加载真正ab名
        ...
        {
            ManifestItem item = this.config.Manifest.GetManifest(assetName);
        ...
        private Object LoadFormAssetBundle(ManifestItem item, Type t)
        {
            ...
            {
                item = this.config.Manifest.GetManifest(item.AB);
            }
        ...
        public T Load<T>(string path) where T : UnityEngine.Object
        {
            ...
            //1.依赖路径
            var item = config.Manifest.GetManifest(path);
            if (item != null)
            {
                //加载依赖
        ...
        {
            var item = config.Manifest.GetManifest(path);
            string realPath = string.IsNullOrEmpty(item.AB) ? item.Path : item.AB;
            //加载assetbundle
        ...
        public int AsyncLoad<T>(string path, Action<T> callback) where T : UnityEngine.Object
        {
            ...
            //获取依赖
            var mainItem = config.Manifest.GetManifest(path);
            if (mainItem != null)
            {
                //依赖任务
        ...
        public string[] GetAssets(string floder, string searchPattern = null)
        {
            ...
            var str = floder + "/";
            foreach (var key in this.config.Manifest.ManifestMap.Keys)
            {
        ...
        void DoNextTask()
        {
            ...
            if ((curDoTask == null || curDoTask.IsComplete) && this.allTaskGroupList.Count > 0)
            {
                ...
                var item = config.Manifest.GetManifest(curDoTask.MainAsset);
        ...
        public void UnloadAsset(string name, bool isForceUnload = false)
        {
            if (name != null)
            {
                var res = config.Manifest.GetDependenciesByName(name);
                if (res == null)
                    return;
                //将所有依赖,创建一个队列 倒序加载
