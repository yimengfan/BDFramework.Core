...
namespace BDFramework.UnitTest
{
    ...
    static public class APITest_Sqlite
    {
        ...
        {
            Assert.StartWatch();
            var ds = SqliteHelper.DB.GetTableRuntime().Where("Id >= 9000").OrderByDesc("Id").FromAll<APITestHero>();
            var time = Assert.StopWatch();
            //降序检测
            bool isPass = true;
        ...
        static public void MultiSelect_OrderBy()
        {
            ...
            var ds = SqliteHelper.DB.GetTableRuntime().Where("Id >= 9000").OrderBy("Id").FromAll<APITestHero>();
