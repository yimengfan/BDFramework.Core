...
namespace Game.demo6_UFlux.CustomCponentBindAdaptor
{
    ...
    /// props
    /// </summary>
    public class Props_Windows : ARenderDataBase
    {
        /// <summary>
        /// 列表的数据结构
        ...
        public PropsList<RD_ItemTest002>  CotentList =new PropsList<RD_ItemTest002>();
    }
    ...
    public class Window_CustomCponentBind : AWindow<Props_Windows>
    {
        ...
        {
            var item = new RD_ItemTest002();
            int id = this.RenderData.CotentList.Count;
            //填充数据 
            ...
            //
            this.RenderData.CotentList.Add(item);
        ...
        {
            if(this.RenderData.CotentList.Count==0)return;
            //随机修改一个数据
            var idx = (int)UnityEngine.Random.Range(0, this.RenderData.CotentList.Count);
            var item = this.RenderData.CotentList.Get(idx);
            var imgIdx = (int)UnityEngine.Random.Range(1,10);
            item.ItemImg = "Image/" + imgIdx;
            //设置好修改数据
            this.RenderData.CotentList.SetChangedData(item);
            this.CommitRenderData();
        }
        ...
        private void Onclick_RemoveItem()
        {
            if(this.RenderData.CotentList.Count==0)return;
            var idx = (int)UnityEngine.Random.Range(0, this.RenderData.CotentList.Count);
            this.RenderData.CotentList.RemoveAt(idx);
            this.CommitRenderData();
        }
