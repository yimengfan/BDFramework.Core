...
// 代码异味追踪机制
// Spotting a problem ≠ fixing it immediately. Separate "discovery" from "repair" to avoid scope creep.

### 6.1 两条规则

1. **编译链内发现 → 直接修。**
   编译链 = 当前改动的文件 + 因 import/依赖该文件而报编译错误的文件。从当前文件开始，修完编译，编译器报下一个错就修下一个，直到编译通过。不要人为扩大范围。

2. **编译链外发现 → 只记录，不修。**
   在阅读无关代码（不在当前编译链中）时发现违规，追加到 `.agent_memory/code_smells.md`，不要顺手改。改了会导致 PR 范围失控、回归风险不可控。

### 6.2 追踪文件格式

文件路径：`<project-root>/memory/code_smells.md`
...
| 2 | Editor/Tools/Foo.cs | L108 | 命名误导 | 方法名 DoX 实际做 Y |
```

### 6.3 追踪文件规则

- **最多 10 条**。新增第 11 条时，删除最旧的一条（FIFO 淘汰）。
- **按严重程度排序**。最严重的排最前：静默吞错误 > 命名不一致 > 冗余代码。
- **每条包含**：文件路径（相对仓库根）、行号、违反的规则名、一句话简述。
- **修复后删除**。当异味被修复（不论是在哪个 session 中），从列表中移除。
- **每次任务结束前**，提醒用户当前待处理条目数，建议安排专门 session 清理。

### 6.4 常见 Code Smell 清单

以下为本项目重点关注的异味类型（不限于此）：

| 优先级 | 异味类型 | 说明 |
|---|---|---|
| 🔴 高 | 静默吞错误 | `catch {}`、`if (x == null) return` 无日志无上报 |
| 🔴 高 | Trusted 路径降级处理 | 内部数据用了 `TryCatch` 降级而非 `throw` |
| 🟡 中 | 命名误导 | 方法/变量名与实际行为不符（如 `ScriptLoder` 拼写错误） |
| 🟡 中 | 职责越界 | Editor 逻辑混入 Runtime 程序集，缺少条件编译隔离 |
| 🟢 低 | 冗余代码 | 未使用的 using、死代码、注释掉的代码块 |
| 🟢 低 | 风格不一致 | 与周围代码风格明显不同的新代码 |
