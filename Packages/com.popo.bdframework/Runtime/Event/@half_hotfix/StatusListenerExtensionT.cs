...
namespace BDFramework.DataListener
{
    ...
    static public class StatusListenerExtensionT
    {
        ...
        static public void AddListener<T>(this AStatusListener dl, Action<T> action = null) where T : class, new()
        {
            dl.AddListener(typeof(T).FullName, action, -1, -1, false);
        }
        /// <summary>
        /// 状态监听泛型版本
        /// </summary>
        ...
        {
            dl.AddListener(typeof(T).FullName, action, order, triggerNum, isTriggerCacheData);
        }
