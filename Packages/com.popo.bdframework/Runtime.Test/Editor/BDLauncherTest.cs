...
namespace Runtime.Test.Editor
{
    ...
    /// 顺序调用 Runtime.Test/Runtime/APITest 下的启动器、AOT 启动与热更装载顺序断言并写出稳定报告。
    ...
    /// sequentially invoking the launcher, AOT-startup, and hotfix-load-order assertions under Runtime.Test/Runtime/APITest and writing a stable report.
    /// </summary>
    public static class BdLauncherBatchBridge
    {
        ...
        public static void RunBatchVerification()
        {
            ApiTestLog.LogTestPurposeAndMeans(
                ...
                "验证启动器反射契约、AOT 启动 StreamingAssets 与热更装载顺序规则、默认执行顺序与 E2E 自动检测入口保持稳定。",
                "顺序执行 Runtime APITest 启动器、AOT 启动与热更装载顺序断言，写出批验证报告，并使用显式退出码反馈结果。"
            );
            ...
            var checks = new (string Name, Action Action)[]
            {
                ...
                (
                    nameof(BdLauncherApiTest.LoadHotfixAssemblies_ShouldRespectKnownDependencyOrder),
                    () => ExecuteWithSetUp(
                        runtimeTest,
                        nameof(BdLauncherApiTest.LoadHotfixAssemblies_ShouldRespectKnownDependencyOrder),
                        runtimeTest.LoadHotfixAssemblies_ShouldRespectKnownDependencyOrder)
                ),
                (
