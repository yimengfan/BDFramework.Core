"""BuildClientPackage batchmode 路径解析和日志辅助函数测试。

测试覆盖范围：
1. Unity 编辑器路径规范化：验证版本化路径模板按声明顺序展开。
2. 项目目录解析：验证缺失 Unity 标记文件时拒绝解析。
3. Unity 可执行文件解析：验证环境变量覆盖、回退候选、dry-run 模式等场景。
4. 日志路径解析：验证 CI 构建元数据将日志重定向到共享 CI 日志根目录。
5. 日志流式传输：验证部分行缓冲直到遇到换行符。
6. batchmode 执行：验证 dry-run 模式、成功/失败完成标记终止卡住的进程。
"""
...
def test_resolve_project_dir_rejects_missing_markers(
    ...
    """验证当必需的 Unity 标记文件（Assets/Packages/ProjectSettings）缺失时，项目目录解析失败。"""
...
def test_resolve_unity_executable_prefers_existing_env_path(
    ...
    """验证已存在的 UNITY_PATH 环境变量覆盖优先于回退候选路径发现。"""
...
def test_resolve_unity_executable_falls_back_after_invalid_env_path(
    ...
    """验证无效的 UNITY_PATH 环境变量会回退到发现的编辑器候选路径。"""
...
def test_resolve_unity_executable_allow_missing_returns_first_candidate(
    ...
    """验证 dry-run 模式可以使用第一个候选路径，即使该路径不存在。"""
...
def test_run_batchmode_dry_run_prints_command_and_skips_execution(
    ...
    """验证 dry-run batchmode 仅打印命令而不会启动 Unity 进程。"""
    messages: list[str] = []
    command = [
