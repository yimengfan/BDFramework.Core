...
3. 默认优先读取 `Packages/com.popo.bdframework/Editor.DevOps~/BuildTools/buildtools.toml`，整个 BuildTools 共用同一份配置。
...
- 这些配置统一放在 `Packages/com.popo.bdframework/Editor.DevOps~/BuildTools/buildtools.toml`，统一通过 `Common/buildtools_config.py` 读取；现有上传模块和 `build_xcode.shell` 都已经接到这套入口上。
...
3. BuildTools 单元测试统一放在 `Packages/com.popo.bdframework/Editor.DevOps~/BuildTools/tests/`，不要挂到 `.test-DevOps/GameFileServer/tests/` 下。
...
推荐在 `Packages/com.popo.bdframework/Editor.DevOps~/BuildTools/buildtools.toml` 中配置：
...
4. `Packages/com.popo.bdframework/Editor.DevOps~/BuildTools/buildtools.toml`
5. `Packages/com.popo.bdframework/Editor.DevOps~/BuildTools/buildtools.toml.example`

## 给其他模块调用

如果调用脚本位于 `Packages/com.popo.bdframework/Editor.DevOps~/BuildTools/` 的子目录，按上面的模块加载规范导入：
...
- `Packages/com.popo.bdframework/Editor.DevOps~/BuildTools/buildtools.toml`
- 也支持统一环境变量覆盖：`BUILDTOOLS_CONFIG`
- 如果正式配置还没生成，则回退到 `Packages/com.popo.bdframework/Editor.DevOps~/BuildTools/buildtools.toml.example`
...
- Assetbundle：`prepare_assetbundle_upload_source()` 只保留运行时真正需要的 `assets.info`、可选的 `assets_subpack.info` 和 `art_assets/*` hash payload；如果顶层 `assets.info` 没带出真实 `art_assets/*` payload，则回退读取 `art_assets/art_assets.info` 补齐并重写 staging 的 `assets.info`；`assets_subpack.info` 只原样上传，不参与资源清单解析；同时会统一过滤 `package_build.info`、`art_assets/buildlogtep.json`、`art_assets/EditorBuild.Info`、`build_result.info` 等构建期元数据，校验 hash 文件集合与 `assets.info` 一致，且声明中存在真实 `art_assets/*` payload
...
```bash
python3 Packages/com.popo.bdframework/Editor.DevOps~/BuildTools/Common/artifact_uploader.py \
...
`artifact_uploader.py` 的单元测试位于：`Packages/com.popo.bdframework/Editor.DevOps~/BuildTools/tests/test_artifact_uploader.py`

执行命令：

```bash
python -m pytest -q Packages/com.popo.bdframework/Editor.DevOps~/BuildTools/tests/test_artifact_uploader.py
```
...
- `Packages/com.popo.bdframework/Editor.DevOps~/BuildTools/tests/test_artifact_uploader_remote.py`

执行命令：

```bash
python -m pytest -q -s Packages/com.popo.bdframework/Editor.DevOps~/BuildTools/tests/test_artifact_uploader_remote.py --run-remote-artifact-tests
```

说明：

- 这个测试会读取 `Packages/com.popo.bdframework/Editor.DevOps~/BuildTools/buildtools.toml` 的 `[artifact_file_server]` 和 `[tests.remote_artifact]`，并真实上传文件到当前配置的远端服务器。
...
1. 远端目录规则、配置解析、上传回调或错误恢复逻辑变更后，必须同步更新并执行 `Packages/com.popo.bdframework/Editor.DevOps~/BuildTools/tests/test_artifact_uploader.py`。
2. 真实上传、远端列表校验或下载回读逻辑变更后，必须执行 `python -m pytest -q -s Packages/com.popo.bdframework/Editor.DevOps~/BuildTools/tests/test_artifact_uploader_remote.py --run-remote-artifact-tests`。
