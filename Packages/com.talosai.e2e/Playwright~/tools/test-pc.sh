...
PLAYER_LAUNCH_ARGS=("-talosPort" "${UNITY_PORT}" "-talosForceE2E" "-screen-fullscreen" "0" "-screen-width" "1280" "-screen-height" "720" "-logFile" "-")
    ...
    if ${IS_WINDOWS_GIT_BASH} && command -v powershell.exe >/dev/null 2>&1 && command -v cygpath >/dev/null 2>&1; then
        ...
        APP_PID="$({
            powershell.exe -NoProfile -Command "\
                \$proc = Start-Process -FilePath '${EXE_PATH_WIN}' -WorkingDirectory '${EXE_DIR_WIN}' -ArgumentList @('-talosPort','${UNITY_PORT}','-talosForceE2E','-screen-fullscreen','0','-screen-width','1280','-screen-height','720','-logFile','${PLAYER_LOG_FILE_WIN}') -PassThru; \
