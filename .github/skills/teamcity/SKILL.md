...
## ⚠️ CRITICAL: Copilot Execution Pattern / 关键：Copilot 执行模式

// ⚠️ 关键规则 —— 违反此规则会导致会话崩溃和 token 浪费
// ⚠️ CRITICAL RULE — Violation causes session collapse and token waste

**MANDATORY PATTERN / 必须遵循的模式**:

When using this skill to trigger builds (`run-build`, `run-build-group`, `run-talos-baseflow-chain`):
// 使用此技能触发构建时：

1. **ALWAYS use `run_in_terminal` with `mode=async`**
   // **必须使用 `run_in_terminal` 的 `mode=async`**

2. **WAIT for the terminal completion notification** — do NOT poll
   // **等待终端完成通知** —— 不要轮询

3. **Call `get_terminal_output` ONCE after notification**
   // 收到通知后调用 `get_terminal_output` **一次**

**FORBIDDEN PATTERNS / 禁止模式**:

- ❌ `mode=sync` for builds >5 minutes
- ❌ Repeated `get_terminal_output` calls without notification
- ❌ `pylanceRunCodeSnippet` + `time.sleep()` polling
- ❌ Shell/python loops polling TeamCity API
...
# Step 1: Launch async
run_in_terminal(
    command=".venv/bin/python .github/skills/teamcity/scripts/update_project_settings.py run-build --wait ...",
    mode="async",
    timeout=1800000  # 30 minutes
)

# Step 2: Wait for notification (system will notify you)
# Step 3: Read result ONCE
