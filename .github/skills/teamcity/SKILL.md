...
- Agent 不得自己写 TeamCity 等待循环。
- Agent 不得在 helper 等待期间反复读取终端输出当作进度轮询。
- Agent 不得使用 `pylanceRunCodeSnippet`、`time.sleep()`、shell/Python 循环、TeamCity REST 查询循环或 `tc_build_poller.sh` 做外层等待。
- Agent 不得在 `run-build --wait` 已经运行时并行启动第二套 TeamCity 轮询。
- Agent 不得先无 `--wait` 触发构建，再自己轮询该 build ID，除非用户明确要求只触发不等待。
