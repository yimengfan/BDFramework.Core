...
测试程序集（`BDFramework.Test`）只能在 Debug 构建中被注入 HybridCLR `hotUpdateAssemblies`；Release 构建必须调用 `EnsureTestAssembliesRemoved()` 确保不包含测试 DLL。新增构建入口必须遵守此分离策略。
...
| `InjectTestAssemblies()` | ✅ 注入 | ❌ 不调用 |
| `EnsureTestAssembliesRemoved()` | ❌ 不调用 | ✅ 构建前强制移除 |
| `ValidateNoTestAssembliesInOutput()` `isReleaseBuild` | `false` → LogWarning | `true` → 抛异常中断 |
| 热更产物含 `.dll.bytes` / `.zlua.bytes` | ⚠️ 警告继续 | ❌ 异常中断 |

### 验收条件

任何涉及测试程序集或构建模式的改动，必须同时满足：
1. Debug 构建热更产物包含 `BDFramework.Test`（`.dll.bytes` 或 `.zlua.bytes` 形式）
2. Release/Profiler 构建热更产物不含 `BDFramework.Test`
3. Release/Profiler 检测到泄漏必须抛异常中断，不得降级为警告
4. Debug 构建中泄漏只产生 `LogWarning`
5. 新增测试程序集时同步更新 `HotfixTestAssemblyInjector.TestAssemblyNames`
6. 新增构建入口时必须对 Release/Profiler 调用 `EnsureTestAssembliesRemoved()` 和 `ValidateNoTestAssembliesInOutput()`
