...
- 退出：所有变更文件的注释已更新；C1 激活时测试已补全；C7 激活时文档已同步
- 回环：发现遗漏依赖 → 回到阶段 A

**阶段 C 验证**
- 前置：阶段 B 退出条件已满足
- 动作：C2 → 运行命中模块本地测试；C3 → diff 复查后提交推送（C6 激活时 `.test-DevOps` 独立仓库单独处理）；C4 → 触发受影响 TeamCity 构建，等待完成并记录 build ID/URL/status
- 退出：已激活的条件门全部通过；commit SHA 已记录；未激活门已说明原因
- 回环：→ 阶段 B（修复代码/测试），修复后补测重跑

**阶段 D 收口**
- 前置：阶段 C 退出条件已满足
- 动作：逐项对照完成前检查（§7）；汇报改动范围 + 测试结果 + commit SHA + 远端证据 + 未运行项原因 + 剩余风险
- 退出：完成前检查全部通过（条件门未激活项已说明原因）
- 回环：未通过 → 回到对应阶段处理
...
- [ ] C1 激活时：已补自动化测试（或已说明原因与替代验证）
- [ ] C7 激活时：需要同步的文档、日志和测试已更新

**阶段 C 验证**
- [ ] C2 激活时：已运行本地测试，全部通过（无法运行时已说明原因并执行替代验证）
- [ ] C2 激活时本地失败已完成根因分析、修复、补测和重跑
- [ ] C3 激活时：已提交推送，提交只包含当前任务相关文件
- [ ] C6 激活时：`.test-DevOps` 已独立提交推送
- [ ] 已记录 commit SHA
- [ ] **C4 条件门**：已判断是否需要远端验证（列出受影响 buildType）
  - [ ] 需要时：已触发 TeamCity、等待完成并通过、已记录 build ID/URL/status、已执行制品验证
  - [ ] 不需要时：已在收口中说明原因

**阶段 D 收口**
- [ ] 任务期间的用户纠正已分类处理（§4 PDCA）
- [ ] `.agent_memory/` 反映真实当前状态
- [ ] 最终回复包含：改动范围、测试结果、commit SHA、远端证据（C4 激活时）、未运行项原因和剩余风险
